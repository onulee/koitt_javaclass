...
public class Ex1006_03 {
	...
	public static void main(String[] args) {
		...
		// 메뉴반복
		loop1: while (true) {
			firstPrint(count); // 메뉴출력메소드
			choice = scan.nextInt();

			switch (choice) {
			case 1:
				count = inputScore(stu, count); // 학생성적 입력
				break;
			case 2:
				printScore(stu, count); // 학생성적 출력
				break;
			case 3:
	...
	// 성적수정
	static void modifyScore(Student[] stu, int count) {
		Scanner scan = new Scanner(System.in);
		String name = "";
		int choice = 0;
	...
	// 등수처리
	static void rankScore(Student[] stu, int count) {
		int rank_num = 1;
	...
	// 학생성적 출력
	static void printScore(Student[] stu, int count) {
	...
	// 학생성적 입력
	static int inputScore(Student[] stu, int count) {
		Scanner scan = new Scanner(System.in);
	...
	static void firstPrint(int count) {
		System.out.println("----------------------");
